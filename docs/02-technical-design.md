...
1. 알림 접근에서는 삼성 메시지 패키지만 허용하고 그룹 요약은 제외한다. SMS 경로에서는 권한·카드 끝 4자리 필터를 적용한다.
2. 묶음 대화 알림은 메시지 배열을 시간순으로 순회하고 각 본문을 독립적으로 파싱한다.
3. 파싱 결과를 검증한다. 실패하면 원문 없이 실패 횟수만 기록한다.
4. 거래시각·상호명·금액·메시지 게시 시각(밀리초)으로 출처 ID를 만들고 같은 수신 이벤트만 제외한다.
...
- `NotificationListenerService`는 `com.samsung.android.messaging`만 처리한다. 신한 슈퍼SOL과 다른 앱 알림은 즉시 무시한다.
- 알림 접근이 처음 연결되거나 다시 연결되면 현재 활성 상태인 삼성 메시지 알림도 한 번 검사해 권한 허용 전 수신된 묶음 알림을 복구한다.
- `MessagingStyle` 메시지 배열이 있으면 각 메시지의 내부 밀리초 시각을 사용한다. 배열이 없으면 확장 본문과 `notification.when`을 사용한다.
- 원본 삼성 메시지 알림은 취소하지 않는다.
...
6. 통과한 행의 `cardLast4`에는 사용자가 설정한 정확한 4자리를 연결한다. 전체 PAN·원문 SMS는 저장하지 않는다.
