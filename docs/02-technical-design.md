...
- Google Play로 설치된 앱은 Android `AppUpdateManager`로 실행·화면 복귀 시 새 버전을 확인한다.
- 일반 업데이트는 Flexible 흐름으로 Google Play의 동의 화면을 열고 백그라운드 다운로드를 시작한다. 다운로드가 끝나면 네이티브 확인창에서 사용자가 재시작을 선택해 적용한다.
- 디버그·직접 설치 APK는 Play 소유 앱이 아니므로 업데이트가 없다는 결과가 정상이다. 이 기능의 실기기 검증은 이전 Play 테스트 버전에서 다음 Play 테스트 버전으로 업데이트할 때 진행한다.
...
4. 거래시각·상호명·금액·메시지 게시 시각(밀리초)으로 출처 ID를 만들고 같은 앱의 수신 이벤트만 제외한다. 서로 다른 허용 앱에서 90초 안에 같은 승인 키가 수신되면 하나로 합치고, 해당 짝은 이후 중복 판단에 재사용하지 않는다.
