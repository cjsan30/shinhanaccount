...
현재 Google Play 비공개 테스트로 배포·검증 중입니다.
...
- 삼성 메시지·신한 SOL·신한 SOL Pay 알림 기반 승인 내역 인식과 중복 방지
- 불완전한 승인 정보의 확인 등록 및 취소 승인-기존 결제 안전 매칭
...
| 주제 | 선택과 이유 |
| --- | --- |
| 결제 인식 | Android 알림 접근으로 삼성 메시지·신한 계열 앱의 승인 알림을 기기 안에서 처리하고, 동일 승인 후보는 시각·상호·금액 기준으로 중복 방지 |
| 불확실한 알림 | 카드 끝 4자리는 일치하지만 금액·시각·가맹점 정보가 부족한 알림은 자동 차감하지 않고 사용자가 확인 후 등록 |
