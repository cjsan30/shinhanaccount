...
export function BackupRestore({ ledger, policyBook, merchantRules, onRestore, notify }: Props) {
  ...
  const restore = async () => {
    ...
    if (!window.confirm('현재 정책과 결제 내역을 모두 백업 파일 내용으로 교체합니다. 계속할까요?')) return;
  ...
  return <section className="backup-card"><h3>백업 · 복원</h3><p>기기 변경 또는 초기화 전에 암호화 백업 파일을 보관하세요. 복원하면 현재 앱 데이터는 백업 내용으로 교체됩니다.</p><label>백업 비밀번호<input aria-label="백업 비밀번호" type="password" value={exportPassword} onChange={(event) => setExportPassword(event.target.value)} placeholder="8자 이상" /></label><button className="sheet-action" onClick={() => void download()}>암호화 백업 저장</button><label className="file-picker">백업 파일 선택<input aria-label="백업 파일" type="file" accept=".shb,application/json" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label>{file && <p className="backup-file">선택됨: {file.name}</p>}<label>복원 비밀번호<input aria-label="복원 비밀번호" type="password" value={restorePassword} onChange={(event) => setRestorePassword(event.target.value)} /></label><button className="sheet-action secondary-action" disabled={!file || !restorePassword} onClick={() => void restore()}>백업으로 복원하기</button></section>;
}
