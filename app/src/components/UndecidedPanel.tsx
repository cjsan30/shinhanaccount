...
export function UndecidedPanel({ entries, items, onClassify }: Props) {
  ...
  return <section className="undecided-panel">
    ...
    <div className="undecided-bulk__action"><label>분류 항목<select aria-label="미정 지출 일괄 분류" value={itemKey} onChange={(event) => setItemKey(event.target.value)}>{items.map((candidate) => <option key={candidate.key} value={candidate.key}>{candidate.bucket === 'resident' ? '정주비' : '학습공간비'} · {candidate.label}</option>)}</select></label><label className="undecided-rule"><input type="checkbox" checked={saveRule} onChange={(event) => setSaveRule(event.target.checked)} disabled={selected.length !== 1 || item.ledgerCategories[0]?.startsWith('custom:')} /> 이 상호명도 앞으로 자동 분류</label><small>{item.ledgerCategories[0]?.startsWith('custom:') ? '사용자 항목은 직접 분류에만 사용합니다.' : selected.length === 1 ? '선택한 한 건의 상호명에만 개인 규칙을 저장합니다.' : '자동 분류 규칙은 한 건을 선택했을 때만 저장할 수 있습니다.'}</small><button className="sheet-action" disabled={!selected.length} onClick={() => onClassify(selected, item, saveRule)}>선택한 {selected.length}건 분류</button></div>
  </section>;
}
