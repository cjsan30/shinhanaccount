...
export function OnboardingFlow({ onComplete }: { onComplete: (payload: CompletePayload) => void }) {
  ...
  return <main className="first-run">
    ...
    <p>정책을 확정했습니다. 현재 사용 내역을 가져오거나, 바로 지원금 관리를 시작할 수 있습니다.</p>
    ...
    <section className="first-run__card onboarding-choice">
      <strong>결제한 적이 있어요</strong><p>신한카드 엑셀 또는 신한은행 PDF를 가져와 현재 내역을 등록합니다.</p>
    ...
    {showGuide && <section className="onboarding-guide"><strong>가져오는 방법</strong><ol><li>신한카드 앱에서 카드 이용내역을 엑셀로 내려받습니다.</li><li>또는 신한 SOL에서 지원금 계좌 거래내역 PDF를 받습니다.</li><li>앱의 설정 · 데이터 가져오기에서 파일을 선택해 확인합니다.</li></ol><p>카드 내역은 설정한 카드와 앞 3자리가 겹치는 카드가 없을 때만 가져옵니다.</p></section>}
  </main>;
}