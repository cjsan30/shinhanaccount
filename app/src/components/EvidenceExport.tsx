...
export function EvidenceExport() {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 4; attempt += 1) {
        ...
        for (const [index, item] of items.entries()) {
          setProgress({ current: index + 1, total: items.length, stage: `이미지 압축 중${attempt ? ` · 용량 조정 ${attempt + 1}차` : ''}` });
  ...
  const renderGroup = (title: string, group: Group, items: Evidence[]) => <section className="evidence-group"><h3>{title} <span>{items.length}장</span></h3><p>여러 장을 한 번에 고른 뒤, 위·아래 버튼 또는 드래그로 순서를 조정하세요.</p><label className="file-picker">이미지 여러 장 추가<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => addFiles(group, event.target.files)} /></label><div className="evidence-list">{items.map((item, index) => <article key={item.id} draggable onDragStart={() => setDragging({ group, index })} onDragOver={(event) => event.preventDefault()} onDrop={() => reorder(group, index)}><img src={item.url} alt={`${title} 증빙 ${index + 1}`} /><span>{index + 1}. {item.file.name}</span><div className="evidence-order"><button type="button" aria-label={`${title} 증빙 위로 이동`} disabled={index === 0} onClick={() => move(group, index, -1)}>위로</button><button type="button" aria-label={`${title} 증빙 아래로 이동`} disabled={index === items.length - 1} onClick={() => move(group, index, 1)}>아래로</button><button type="button" onClick={() => remove(group, item)}>삭제</button></div></article>)}</div></section>;
