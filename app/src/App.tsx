...
function App() {
  ...
  const updatePolicyDraft = (item: PolicyItem, value: string) => {
    ...
    setPolicyDraft((current) => current ? { plans: { ...current.plans, [item]: Math.floor(amount) } } : current);
  };
  const confirmPolicy = () => {
