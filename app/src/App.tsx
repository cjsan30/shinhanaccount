...
import { getPolicyLimit, loadPolicy, parsePolicyText, savePolicy, type SupportPolicy } from './domain/policy';
...
function App() {
  ...
  const rowsFor = (bucket: BudgetKey): Array<[string, number, number]> => planned[bucket].map(([category]) => [categoryNames[category], policy.plans[bucket][category], ledger.entries.filter((entry) => entry.status === 'classified' && entry.bucket === bucket && entry.category === category).reduce((sum, entry) => sum + entry.amount, 0)]);
