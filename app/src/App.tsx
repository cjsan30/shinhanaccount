...
function App() {
  ...
  const saveMerchantRule = () => {
    ...
    if (!item) return;
    try { setMerchantRules((current) => [...current, createMerchantRule(ruleMerchant, { bucket: item.bucket, category: item.ledgerCategories[0] })]); setRuleMerchant(''); show('다음 결제부터 이 상호명 규칙을 적용합니다. 기존 내역은 바꾸지 않습니다.'); }
