...
type Panel = 'resident' | 'study' | 'undecided' | 'recent' | 'cancel' | 'edit' | 'delete' | 'settings' | 'evidence' | 'payment' | null;
...
function App() {
  ...
  useEffect(() => {
    ...
    setPanel('settings');
    window.setTimeout(() => document.querySelector('.import-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 180);
  ...
  const title = panel === 'resident' ? '정주비 상세' : panel === 'study' ? '학습공간비 상세' : panel === 'undecided' ? '미정 지출' : panel === 'recent' ? '최근 결제' : panel === 'cancel' ? '취소 확인' : panel === 'edit' ? '결제 내역 수정' : panel === 'delete' ? '결제 내역 삭제' : panel === 'settings' ? '설정' : panel === 'evidence' ? '증빙 PDF 만들기' : '직접 지출 등록';
