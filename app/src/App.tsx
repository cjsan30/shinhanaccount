...
type Panel = 'resident' | 'study' | 'undecided' | 'recent' | 'cancel' | 'edit' | 'delete' | 'settings' | 'evidence' | 'payment' | 'import' | 'operations' | 'data' | null;
...
function App() {
  ...
  const saveMerchantRule = () => {
    ...
    if (!item) return;
    try {
      const rule = createMerchantRule(ruleMerchant, { bucket: item.bucket, category: item.ledgerCategories[0] });
      const remaining = merchantRules.filter((current) => current.normalizedMerchant !== rule.normalizedMerchant);
      if (remaining.length >= 50) { show('자동 분류 규칙은 최대 50개까지 저장할 수 있습니다.'); return; }
      setMerchantRules([...remaining, rule]);
      setRuleMerchant('');
      show('다음 결제부터 이 상호명 규칙을 적용합니다. 기존 내역은 바꾸지 않습니다.');
    }
